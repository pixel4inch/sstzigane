...
/* ===================================
    Re usable javascript text trim
 ====================================== */

(function () {
  function trimTextPlugin(selector = ".trim-text") {
    document.querySelectorAll(selector).forEach(el => {
      const limit = parseInt(el.dataset.limit) || 150;
      const link = el.dataset.link || "#";
      const fullText = el.textContent.trim();

      if (fullText.length > limit) {
        const shortText = fullText.substring(0, limit).trim();

        el.innerHTML = `
          ${shortText}...
          <a href="${link}" class="know-more-link">Know More</a>
        `;
      }
    });
  }
  ...
  window.trimTextPlugin = trimTextPlugin;
})();
